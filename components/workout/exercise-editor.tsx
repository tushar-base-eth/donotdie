...
export function ExerciseEditor({
  ...
  const handleDistanceChange = (e: React.ChangeEvent<HTMLInputElement>, setIndex: number) => {
    ...
    const distance_meters = distance; // Assuming input in meters
    const newSets = [...exercise.sets];
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 bg-background/80 backdrop-blur-sm border-t">
            <Button
              onClick={() => {
                const newSet: Set = {
                  id: `temp-set-${exercise.sets.length}`, // Temporary ID
                  workout_exercise_id: exercise.id, // Use existing exercise ID
                  set_number: exercise.sets.length + 1, // Incremental set number
                ...
                onUpdateSets(exerciseIndex, newSets);
                setTimeout(() => {
                  repsInputRefs.current[newSets.length - 1]?.focus();
                }, 0);
              }}
